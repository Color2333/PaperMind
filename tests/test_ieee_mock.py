...
from packages.storage.repositories import IeeeQuotaRepository, PaperRepository
...
class TestIeeeMockIngest:
    ...
    def test_ieee_paper_save_to_db(self, mock_ieee_papers, isolated_db):
        """测试 IEEE 论文保存到数据库"""
    ...
    def test_ieee_paper_query(self, mock_ieee_papers, isolated_db):
        """测试 IEEE 论文查询"""
        with session_scope() as session:
            repo = PaperRepository(session)
...
class TestIeeeQuotaMock:
    ...
    def test_quota_check_without_api(self, isolated_db):
        """测试配额检查（不需要 API）"""
        with session_scope() as session:
            quota_repo = IeeeQuotaRepository(session)
            today = date.today()

            # topic_id 为 None（列允许 NULL，避免 FK 约束）
            topic_id = None
