        ...
        <div className="flex items-center gap-1.5">
          {relevantCats.size > 0 && relevantCount < candidates.length && (
            <button
              ...
              className="text-success hover:bg-success/10 rounded-md px-2 py-0.5 text-[10px] font-medium transition-colors"
            >
              仅选相关
            </button>
          )}
          <button
            onClick={() =>
              setSelected(
                allSelected ? new Set() : new Set(candidates.map((c) => String(c.arxiv_id ?? "")))
              )
            }
            ...
            <label
              ...
              className={cn(
                ...
                isChecked
                  ...
                  : "bg-surface hover:bg-hover border border-transparent",
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start gap-1.5">
                  <p className="text-ink flex-1 leading-snug font-medium">
                    {String(p.title ?? "")}
                  </p>
                  {isRelevant && relevantCats.size > 0 && (
                    <span className="bg-success/10 text-success shrink-0 rounded px-1.5 py-0.5 text-[9px] font-medium">
                      相关
                    </span>
                  )}
                </div>
...
const StepDataView = memo(function StepDataView({
      ...
      <div className="space-y-2">
        ...
        {evidence.length > 0 && (
          <div className="border-border-light border-t pt-2">
            <p className="text-ink-tertiary mb-1 text-[10px] font-medium">
            ...
            <div className="flex flex-wrap gap-1">
              {evidence.slice(0, 8).map((e, i) => (
                <button
                  ...
                  className="bg-surface text-ink-secondary hover:bg-hover hover:text-primary max-w-[200px] truncate rounded px-1.5 py-0.5 text-[9px] transition-colors"
    ...
    return (
      <div className="space-y-1">
        {topics.map((t, i) => (
          <div
            key={i}
            className="bg-surface flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-[11px]"
          >
            ...
            {t.paper_count !== undefined && (
              <span className="text-ink-tertiary">{String(t.paper_count)} 篇</span>
            )}
            {t.enabled !== undefined && (
              <span
                className={cn(
                  "ml-auto rounded px-1.5 py-0.5 text-[9px]",
                  t.enabled ? "bg-success/10 text-success" : "bg-ink/5 text-ink-tertiary"
                )}
              >
                {t.enabled ? "已订阅" : "未订阅"}
              </span>
  ...
  if (toolName === "get_similar_papers") {
    ...
    if (ids.length > 0) {
      return <p className="text-ink-secondary text-[11px]">找到 {ids.length} 篇相似论文</p>;
    }
  }
      ...
      <div className="space-y-1.5">
        {suggestions.map((s, i) => (
          <div key={i} className="bg-surface rounded-lg px-2.5 py-2 text-[11px]">
            ...
            {s.reason !== undefined && (
              <p className="text-ink-tertiary mt-0.5 text-[10px]">{String(s.reason)}</p>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="text-[11px]">
        ...
        {data.novelty !== undefined && (
          <p className="text-ink-secondary mt-1">
            <span className="font-medium">创新点:</span> {String(data.novelty)}
          </p>
        )}
        {data.methodology !== undefined && (
          <p className="text-ink-secondary mt-0.5">
            <span className="font-medium">方法:</span> {String(data.methodology)}
          </p>
        )}
      </div>
    );
  ...
  if (toolName === "reasoning_analysis" && data.reasoning_steps) {
    const steps = Array.isArray(data.reasoning_steps)
      ? (data.reasoning_steps as Array<Record<string, unknown>>)
      : [];
    return (
        ...
        {steps.slice(0, 6).map((s, i) => (
          <div key={i} className="bg-surface rounded-lg px-2.5 py-1.5 text-[11px]">
            ...
            {s.evidence !== undefined && (
              <p className="text-ink-tertiary mt-0.5 truncate text-[10px]">{String(s.evidence)}</p>
            )}
          </div>
        ))}
      </div>
  ...
  if (toolName === "identify_research_gaps" && data.analysis) {
    ...
    const gaps = Array.isArray(analysis.research_gaps)
      ? (analysis.research_gaps as Array<Record<string, unknown>>)
      : [];
    return (
      <div className="space-y-1.5">
    ...
    return (
      <div className="text-[11px]">
        ...
        {data.abstract_zh !== undefined && (
          <p className="text-ink-secondary mt-1 line-clamp-3">{String(data.abstract_zh)}</p>
        )}
  ...
  /* 兜底：原始 JSON */
  return (
    <pre className="bg-surface text-ink-secondary max-h-40 overflow-auto rounded-lg p-2.5 text-[11px]">
